...
def simulate(alpha_dict, region_dict, name, neut, start, stone_bag, tags='None'):
        ...
        for idx, group in enumerate(groups):
            ...
            for progress in progress_urls:
                ...
                try:
                    ...
                    set_alpha_properties(s, alpha_id, name="%s" % name, color=None, tags=tags)
                    logger.info(alpha_id)
    ...
    # logger.info("success rate: %.3f"%(float(len(stone_bag))/len(comb_fields)))
    return stone_bag
...
def get_datasets(
    ...
    return pd.DataFrame(s.get(url).json()['results'])
...
def get_datafields(
    ...
    if len(search) == 0:
        url_template = "https://api.worldquantbrain.com/data-fields?" + f"&instrumentType={instrument_type}" + f"&region={region}&delay={str(delay)}&universe={universe}&dataset.id={dataset_id}&limit=50" + "&offset={x}"
        count = s.get(url_template.format(x=0)).json()['count']
    else:
        url_template = "https://api.worldquantbrain.com/data-fields?" + f"&instrumentType={instrument_type}" + f"&region={region}&delay={str(delay)}&universe={universe}&limit=50" + f"&search={search}" + "&offset={x}"
        count = 100
    datafields_list = []
...
def get_alphas(start_date, end_date, sharpe_th, fitness_th, longCount_th, shortCount_th, region, universe, delay, instrumentType, alpha_num, usage, tag: str = '', color_exclude='', s=None):
    ...
    if usage != "submit":
        ...
        for alpha_detail in alpha_list:
            ...
            if any(checks_df["result"] == "FAIL"):
                ...
                # 通过了最基础的项目
                # 把全部的信息以字典的形式返回
                rec = {"id": id, "type": type, "author": author, "instrumentType": instrumentType, "region": region, "universe": universe, "delay": delay, "decay": decay, "neutralization": neutralization,
                       "truncation": truncation, "pasteurization": pasteurization, "unitHandling": unitHandling, "nanHandling": nanHandling, "language": language, "visualization": visualization, "code": code,
                       "description": description, "operatorCount": operatorCount, "dateCreated": dateCreated, "dateSubmitted": dateSubmitted, "dateModified": dateModified, "name": name, "favorite": favorite,
                       "hidden": hidden, "color": color, "category": category, "tags": tags, "classifications": classifications, "grade": grade, "stage": stage, "status": status, "pnl": pnl,
                       "bookSize": bookSize, "longCount": longCount, "shortCount": shortCount, "turnover": turnover, "returns": returns, "drawdown": drawdown, "margin": margin, "fitness": fitness, "sharpe": sharpe,
                       "startDate": startDate, "checks": checks, "os": os, "train": train, "test": test, "prod": prod, "competitions": competitions, "themes": themes, "team": team, "pyramids": pyramids}
...
def first_order_factory(fields, ops_set):
    ...
    for field in fields:
        ...
        for op in ops_set:
            ...
            elif op == "tsalpha_set += ts_comp_factory(op, field, 'buckets', [5, 10, 15, 20])":
...
def trade_when_factory(op, field, region, delay=1):
    output = []
    open_events = ["ts_arg_max(volume, 5) == 0", 
    ...
    if delay==1:
        exit_events = ["abs(returns) > 0.1", 
                       "-1", 
                       "days_from_last_change(ern3_pre_reptime) > 20"]
    else:
        exit_events = ["abs(returns) > 0.1", 
                       "-1"]
    usa_events = ["rank(rp_css_business) > 0.8", 
                  ...
                  "pcr_oi_270 < 1", 
                  "pcr_oi_270 > 1"]
    asi_events = ["rank(vec_avg(mws38_score)) > 0.8", 
                  "ts_rank(vec_avg(mws38_score),22) > 0.8"]
    eur_events = ["rank(rp_css_business) > 0.8", 
                  "ts_rank(rp_css_business, 22) > 0.8",
...
def group_factory(op, field, region):
    ...
    if region == "usa" or region.lower() == "usa":
        ...
        groups += usa_group_1 + usa_group_3 + usa_group_7
